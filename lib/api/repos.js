...
repos._getBuildFilesHandler = function( error, result, currentBuildSha, parentBuildSha, iteration ) {
  ...
  if ( error || !repos.responseObject.builds ) {
    ...
    result.files.forEach( function( value ) {
      var currentNode = repos.responseObject.nodes.filter( function( node ) {
        return ( node.name === value.filename && node.type === 'file'  );
      } ),
          currentLink = repos.responseObject.links.filter( function( link ) {
        return (
          link.source === currentBuild.number &&
          link.target === value.filename
        );
      } ),
          buildStatus = currentBuild.status ? 'passed' : 'failed';

      // check if it is a new new
      if ( !currentNode.length ) {
        ...
        newNode[ buildStatus ] = 1;

        repos.responseObject.nodes.push( newNode );
      } else {
        // set counter for connected build status
        if ( currentNode[ 0 ][ buildStatus ] ) {
          currentNode[ 0 ][ buildStatus ]++;
        } else {
          currentNode[ 0 ][ buildStatus ] = 1;
        }
      }
      ...
      if ( !currentLink.length ) {
        ...
        currentLink[ 0 ].commits++;
      }

      // count commit change details in builds object
      currentBuild.changes = currentBuild.changes || 0;
      currentBuild.changes += value.changes;

      currentBuild.additions = currentBuild.additions || 0;
      currentBuild.additions += value.additions;

      currentBuild.deletions = currentBuild.deletions || 0;
      currentBuild.deletions += value.deletions;
    } );
