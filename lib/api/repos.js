...
repos._getBuildFilesHandler = function( error, result, currentBuildSha, parentBuildSha ) {
  ...
  if ( error ) {
    ...
    console.log( result.sha + ' | ' + currentBuildSha + ' | ' + parentBuildSha );
      ...
      if ( result.sha === parentBuildSha ) {
        ...
        if ( result.parents.length ) {
          var message = {
            user : responseObject.github.owner.login,
            repo : responseObject.github.name,
            sha  : result.parents[0].sha
          };

          console.log( 'GET COMMIT: ' + result.parents[0].sha );
          github.repos.getCommit(
            message,
            function( error, result ) {
              repos._getBuildFilesHandler.apply( this, [ error, result, currentBuildSha, parentBuildSha ] );
            }.bind( this )
          );
        }
      }
